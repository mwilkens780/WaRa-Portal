@extends('layouts.app')
@section('title', 'Wettkämpfe – ' . $child->name)
...
<div class="mt-2 space-y-4">
        ...
        <div x-show="open" x-cloak class="border-t border-gray-100">
            ...
            <div x-show="tab === 'signup'" x-cloak class="p-5 space-y-3">
                ...
                @if($signupRequest->isActive() && $response)
                    <form method="POST"
                          action="{{ route('parent.child.signup.respond', [$child->id, $signupRequest]) }}"
                ...
                @elseif($response)
                    <div class="pt-4 border-t border-gray-100">
                        ...
                        @endif
                        @if($response->carpool_seats)
                            <p class="text-xs text-gray-500 mt-1">Fahrgemeinschaft: {{ $response->carpool_seats }} freie Plätze angeboten</p>
                        @endif
                    </div>
                @else
                    <div class="pt-4 border-t border-gray-100">
                        <p class="text-xs text-gray-400">Keine Anmeldung für {{ $child->firstname }} hinterlegt.</p>
                    </div>
                @endif
            </div>
            @endif
    ...
    @empty
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 px-5 py-12 text-center text-gray-400">
            Noch keine Wettkämpfe für {{ $child->firstname }} zugewiesen.
        </div>
    @endforelse
