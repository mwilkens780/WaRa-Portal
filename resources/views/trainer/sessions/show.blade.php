...
<div class="mt-2 space-y-6" x-data="{ activeTab: 'attendance' }">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
        <div class="flex flex-wrap items-start justify-between gap-4">
            ...
            <div class="flex gap-2 flex-shrink-0 flex-wrap" x-data="{ showSub: false }">
                {{-- Vertretung --}}
                <div class="relative">
                    <button type="button" @click="showSub = !showSub"
                            class="px-3 py-2 border border-amber-300 bg-amber-50 rounded-lg text-sm text-amber-700 hover:bg-amber-100 transition-colors font-medium">
                        Vertretung
                    </button>
                    <div x-show="showSub" x-cloak @click.outside="showSub = false"
                         class="absolute right-0 top-full mt-1 w-64 bg-white rounded-xl shadow-lg border border-gray-200 p-4 z-20">
                        <p class="text-xs font-semibold text-gray-600 mb-2">Trainer für diese Einheit ändern</p>
                        <form method="POST" action="{{ route('trainer.sessions.substitute', $session) }}">
                            @csrf @method('PATCH')
                            <select name="trainer_id" required
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm mb-2 focus:ring-2 focus:ring-amber-400 outline-none">
                                @foreach($allTrainers as $t)
                                    <option value="{{ $t->id }}" {{ $session->trainer_id == $t->id ? 'selected' : '' }}>
                                        {{ $t->lastname }}, {{ $t->firstname }}
                                    </option>
                                @endforeach
                            </select>
                            <button type="submit"
                                    class="w-full px-3 py-2 bg-amber-500 text-white text-sm font-medium rounded-lg hover:bg-amber-600 transition-colors">
                                Trainer übernehmen
                            </button>
                        </form>
                    </div>
                    ...
                    <div class="border border-gray-100 rounded-xl overflow-hidden mb-3">
                        ...
                        <div class="bg-gray-50 px-4 py-2.5 flex flex-wrap items-center gap-2">
                            ...
                            @if($block->repetitions && $block->distance)
                                ...
                                <span class="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full font-medium">= {{ number_format($block->repetitions * $block->distance) }} m</span>
                            @endif
