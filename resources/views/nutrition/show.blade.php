@extends('layouts.app')
@section('title', 'Ernährung – ' . $user->name)
@section('page-title', 'Ernährungsberatung: ' . $user->name)
...
<div class="max-w-3xl mt-2 space-y-5">
    ...
    <a href="{{ route('nutrition.index') }}" class="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-primary transition-colors">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
            <svg class="w-5 h-5 text-green-600 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
            <h2 class="text-base font-semibold text-gray-800">Neues Dokument hochladen</h2>
        </div>
        <form method="POST" action="{{ route('nutrition.upload', $user) }}" enctype="multipart/form-data" class="px-6 py-5 space-y-4">
            @csrf
            <div>
            ...
            <div x-data="{
                ...
                <div class="flex flex-wrap gap-1.5 px-3 py-2 border border-gray-300 rounded-lg min-h-[42px] focus-within:ring-2 focus-within:ring-primary/30 focus-within:border-primary">
                    ...
                    <input type="text" x-model="input" @keydown="onKeydown($event)"
                           ...
                           list="tag-suggestions-n">
                    <datalist id="tag-suggestions-n">
            ...
            <button type="submit" class="bg-green-600 hover:bg-green-700 text-white font-semibold px-5 py-2 rounded-lg text-sm transition-colors">
                Hochladen
            </button>
        </form>
    </div>
        ...
        <div class="divide-y divide-gray-50">
            ...
            <div class="flex items-start justify-between px-6 py-3.5 gap-4">
                ...
                <div class="flex items-center gap-2 flex-shrink-0">
                    ...
                    <form method="POST" action="{{ route('nutrition.destroy', $doc) }}" onsubmit="return confirm('Dokument wirklich löschen?')">
                        @csrf @method('DELETE')
